...
internal abstract class AbstractTypedStaticRouter<T> : StaticRouter, IRouteHandler where T : class, IRequestData
{
    ...
    public AbstractTypedStaticRouter(IEnumerable<string> _routeNames, LoggingUtil logger, ConfigUtil config, JsonUtil jsonUtil) : base(jsonUtil, RouteManager.GetRoutes(_routeNames))
    {
        if (Config == null)
        {
            Config = config;
