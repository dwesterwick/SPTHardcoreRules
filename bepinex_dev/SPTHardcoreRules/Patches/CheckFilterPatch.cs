...
{
    public class CheckFilterPatch : ModulePatch
    {
        ...
        {
            return typeof(GClass2174).GetMethod("CanAccept", BindingFlags.Public | BindingFlags.Static);
        }

        [PatchPostfix]
        private static bool PatchPostfix(bool __result, EFT.InventoryLogic.IContainer container, Item item)
        {
            return __result;
            ...
            bool isItemExamined = Examined(container, item);
            ...
            bool containerIsSecured = secureContainers.Any(c => c.TemplateId == container.ParentItem.TemplateId);
            if (containerIsSecured)
            {
                Logger.LogInfo("Container " + container.ParentItem.LocalizedName() + " (" + container.ParentItem.TemplateId + ") is secured");
            ...
            bool isContainerExamined = Examined(container, container.ParentItem);
            bool isContainerWhitelisted = IsWhitelisted(container.ParentItem, isContainerExamined, SPTHardcoreRulesPlugin.IsInRaid);
            if (isContainerWhitelisted)
            {
                Logger.LogInfo("Container " + container.ParentItem.LocalizedName() + " whitelisted: " + isContainerWhitelisted);
            }
