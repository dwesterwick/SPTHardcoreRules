...
namespace SPTHardcoreRules.Patches
{
    public class GetPrioritizedGridsForLootPatch : ModulePatch
    {
        ...
        {
            return typeof(GClass2555).GetMethod("GetPrioritizedGridsForLoot", BindingFlags.Public | BindingFlags.Static);
        }

        [PatchPostfix]
        private static void PatchPostfix(ref IEnumerable<GClass2317> __result, Item item)
        {
            // Remove containers in which the item is blacklisted
            List<string> blacklistedContainerTemplateIDs = new List<string>();
            foreach (GClass2317 container in __result)
            {
